...
public class addServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            if (adminService.insert(img,number,bookName,classify,author,publisher,bookLocation,content,count)){

                Message message = new Message(001,"添加成功");
                String jsonString = JSONObject.toJSONString(message);
                writer.write(jsonString);
            }else {

                Message message = new Message(-1,"添加失败");
                String jsonString = JSONObject.toJSONString(message);
                writer.write(jsonString);
            }
